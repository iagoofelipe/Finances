pyside6-rcc application/src/ui/imgs/resource.qrc -o application/src/ui/auto/resource_rc.py
pyside6-uic application/src/ui/CreateAccountForm.ui -o application/src/ui/auto/ui_CreateAccountForm.py
pyside6-uic application/src/ui/LoginForm.ui -o application/src/ui/auto/ui_LoginForm.py
pyside6-uic application/src/ui/LoginPage.ui -o application/src/ui/auto/ui_LoginPage.py
pyside6-uic application/src/ui/MainPage.ui -o application/src/ui/auto/ui_MainPage.py
pyside6-uic application/src/ui/ConfigForm.ui -o application/src/ui/auto/ui_ConfigForm.py
pyside6-uic application/src/ui/TableForm.ui -o application/src/ui/auto/ui_TableForm.py
pyside6-uic application/src/ui/RegView.ui -o application/src/ui/auto/ui_RegView.py